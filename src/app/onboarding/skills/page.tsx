...
export default function SkillsOnboardingPage() {
  ...
  const options = useMemo(() => {
    ...
    console.log("User professions:", userProfessions); // Debug
    ...
    if (userProfessions.length === 0) {
      ...
      console.log("No professions selected, showing all skills:", arr.length); // Debug
      return arr;
    }
    ...
    userProfessions.forEach(profession => {
      ...
      console.log(`Skills for ${profession}:`, skills); // Debug
      if (skills) {
    ...
    const arr = [...allRelevantSkills];
    for (let i = arr.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [arr[i], arr[j]] = [arr[j], arr[i]];
    }
    console.log("Filtered skills for professions:", arr.length); // Debug
    return arr;
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden">
      ...
      >
        {isClient && options.map((skill, index) => {
